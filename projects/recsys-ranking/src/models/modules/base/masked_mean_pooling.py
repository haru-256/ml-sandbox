...
class MaskedMeanPooling(nn.Module):
    ...
    def forward(
        ...
        # Check if any valid elements exist per batch
        has_valid = padding_mask.any(dim=1, keepdim=True)  # (B, 1)

        # Masked mean: zero out padding positions, sum, then divide by count
        masked_sequence = sequence * padding_mask.unsqueeze(-1).to(sequence.dtype)
        valid_counts = padding_mask.sum(dim=1, keepdim=True)  # (B, 1)
        mean_pooled = masked_sequence.sum(dim=1) / valid_counts.clamp_min(1).to(
            sequence.dtype
        )  # (B, D)

        # Fallback to global embedding when all positions are padding
